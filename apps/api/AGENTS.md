...
- migration `028_project_runtime_contexts` 必须先于包含 Project Runtime Context 的 API/Admin 启动；release manifest migration ceiling 不得低于 `28`。down 仅允许在没有项目配置记录时执行，不能通过删除配置绕过已依赖业务环境的回滚风险。
