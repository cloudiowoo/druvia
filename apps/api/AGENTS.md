...
- 环境限定授权投影使用 `contractVersion: 2`，只通过服务端 `X-Hasura-Druvia-Service-Environment` 与投影视图 JSONB 获准环境键匹配；保留旧 `contractVersion: 1`，但不能让旧合同覆盖已启用的环境限定 baseline。preview/apply 必须确认 runtime context 已启用、匿名 select 已关闭；删除受管 runtime context 时与 Data Access apply/recover 共用项目锁，存在环境限定 baseline 或未终结 v2 operation 时拒绝；target recovery 落库前再检查环境，失效时整批失败关闭。普通 Hasura SELECT 中视图不能依赖 `hasura.user` GUC，不能用缺失环境时的 owner-only 回退代替失败关闭；新增列、权限表达式与合同必须纳入既有依赖和 baseline 校验。完整设计及隔离证据见 `docs/plans/2026-09-23-environment-scoped-authorization-projection.md`。
