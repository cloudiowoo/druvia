...
- Project Runtime Context 已完成本地代码切片：migration `028` 新增独立审计化配置表，服务端仅接受 `local / sandbox / testflight / production`。项目 owner、workspace owner 与 super admin 可经项目设置页或受限 API 更新；Project Session、API Key、Trusted Backend Key 与客户端 Header 均不能声明环境。RPC 在同一事务内设置 `druvia.service_environment`，GraphQL、Functions internal GraphQL、Realtime token 以及 Data Access HTTP/Realtime verifier 由同一项目配置生成 Hasura session variable；公开 GraphQL/WS Nginx 路径清除客户端伪造 Header。无记录维持兼容模式，读取失败返回脱敏 503。活动本地 PostGIS 已实际应用 `028`，release target/ceiling 升至 `28`，并加入镜像构建前测试门禁。单元、API/Admin build 通过；真实 PostgreSQL+Hasura 的两项目交替 RPC/GraphQL/Realtime 联合验收与 PITCHETCH V23 仍待执行，尚未发布 stable 镜像或执行 OTA。
