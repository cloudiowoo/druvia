...
- Project Runtime Context 使用 migration `028_project_runtime_contexts` 的独立、审计化配置表，不能复用项目 schema environment 或可合并 JSON settings。合法值固定为 `local / sandbox / testflight / production`；无记录表示兼容模式，损坏/读取失败必须拒绝而不是回退默认值。仅固定项目管理 capability `runtime_context:manage` 可变更配置。服务端在 RPC transaction 以内先以空值屏蔽连接或角色级 `druvia.service_environment`，再为已启用项目设置受管值；GraphQL、Functions internal GraphQL、Realtime JWT 和 Data Access HTTP/Realtime verifier 只从同一配置派生 `x-hasura-druvia-service-environment`。公开 Nginx 的 GraphQL/WS 直通路径删除客户端同名 Header，API proxy 不接受 caller Header。配置切换不追溯修改已签发 Realtime JWT，刷新 token 后才生效；如果将来需要即时撤销，必须另行设计连接回收。migration down 在存在任一运行环境记录时拒绝；完整 release migration ceiling 为 `28`，但 updater-only bootstrap 仍可使用 migration `027`，因为它仅建立 migration `027+` 的回滚执行能力。
