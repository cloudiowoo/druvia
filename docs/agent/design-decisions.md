...
- Data Access v2 只扩展 authenticated select：`owner` 与当前项目 schema 内单跳 authorization projection 关系固定以 `AND` 组合；actor 始终是 `X-Hasura-User-Id`。不接受客户端 SQL、任意 Hasura permission JSON、Session Header、`OR`、多跳或跨项目关系。
- 项目投影合同必须显式声明 `policyVersion: 2`；旧合同 `contractVersion: 1` 保持不变，环境限定的新合同使用 `contractVersion: 2`，在 view 中声明唯一非 key JSONB `environmentColumn`。其固定 select 谓词增加 `environmentColumn _has_key X-Hasura-Druvia-Service-Environment`，JSONB 仅能包含当前环境获准读取的键，不能用值为 false 的键表示拒绝；应用 migration/verifier 必须先证明这一输出不变量，Druvia 静态 digest 不能代替业务验证。涉及表的匿名 select 必须关闭，且项目受管 runtime context 必须在 preview/apply 时有效；缺少环境 Header 必须失败关闭。Hasura CE v2.48.0 的普通 SELECT 不保证在视图内部提供 `hasura.user` GUC，应用授权投影视图不得依赖该 GUC 判定环境。schema 只能由 `projectId` 派生。view 必须是项目 db_user 拥有的 regular security-barrier view、对 PUBLIC 无表级或列级 privilege、对 Hasura 客户端角色零 CRUD；其递归非系统 relation 依赖必须全部位于 operation 固定的项目 schema，闭包同时包含 rewrite dependency 和全部 inheritance/partition descendant。当前只允许系统函数依赖，直接函数和 custom operator 的 implementation function 都必须检查。排序后的 relation 身份，以及递归 helper view/materialized view 的 definition digest、owner、完整 `reloptions`、表级/列级 PUBLIC ACL 和输出列必须进入 dependency digest；definition 只裁剪首尾空白，不能折叠可能改变 SQL 字面量的内部空白。helper view 也必须由项目 db_user 拥有、无 PUBLIC privilege 且不能启用 `security_invoker`，不能靠 helper、跨 schema partition 或自定义 operator 间接引用其他项目环境或在 apply 后改写授权语义。object relationship mapping 必须完整且仅一次覆盖 view key，并将来源 owner column 映射到目标 actor column；受管关系必须唯一、保持 object 类型并精确匹配完整 Hasura `using` 结构。
- v2 只能通过项目级 preview/apply/recover 激活，单表保存和 adoption 不得创建或覆盖 v2。九表等批量变更对预检时完整 Hasura resource version 使用一次 `replace_metadata`，同时提交 view tracking、relationship 和 authenticated select；任何 metadata 并发变更都必须重新预检，不允许用旧完整 metadata 覆盖，也不允许回退到非原子 metadata bulk。普通 PostgreSQL DDL 不保证推进 Hasura resource version，因此 preview、apply 写入前后和 target recovery 持久化前都复验来源表列能力。同名 relationship 只允许精确复用，不得覆盖 custom/legacy 定义。v2 激活后的列能力 drift 使用受限单表 reconcile：policy version、select constraint、dependency snapshot/digest 必须保持不变，已删除/不可写列从 grants 收缩，任何扩大 grants 的请求都拒绝，新增列默认不授权；reconcile 在 preview、apply 接纳、metadata 写入前和 baseline 提交前均重算并匹配依赖 digest，任一阶段漂移立即失败关闭。drift 未处理前新的项目 projection preview 失败关闭。
- 一个项目的最新授权投影合同必须覆盖该 schema 内全部现有 v2 baseline；可以在完整旧集合上增加表，但不能通过较小合同遗留无法从最新项目 operation 恢复的旧 v2 表。
- 已存在环境限定的 v2 baseline 时，旧 `contractVersion: 1` 不得覆盖；禁用项目 runtime context 必须与 Data Access projection apply/recover 使用同一项目 advisory lock，并在锁内拒绝仍被环境限定 baseline 或未终结 v2 operation 使用的配置（409）；target recovery 落 baseline 前必须复核环境，失效时整批失败关闭，避免配置删除与 metadata 更新交错或之后持续读失败。
- 每个 v2 baseline 保存 policy、显式字段 grants、完整 permission snapshot、项目合同和数据库/metadata dependency digest。view definition、递归 relation 依赖、owner、完整 `reloptions`、列、key、mapping 或 relationship 漂移时进入 `dependency_invalid`；`security_invoker=true` 始终拒绝。只有项目最新成功 apply 的批次可通过显式项目别名确认进入整批恢复；`target_resource_version` 作为成功 apply 的持久标记，即使该批次后来 fail-closed 或当前 metadata 恰好回到旧 source，也必须统一失败关闭并继续阻断更早批次重放，不能降级解释为未应用。无法证明完整 target 时统一移除受影响表的 authenticated select，并只允许基于该失败关闭 provenance 的新批量预检重新开放。活动状态优先未完成 writer 以及最近成功/失败关闭证据；`superseded`、失败或尚未应用的 preview 不得遮蔽或清除此 provenance，只有更新且成功完成的批次可以替代。completed recovery 在同一事务中先 supersede 未应用 preview 再 claim，claim 失败必须恢复 preview 状态，避免唯一索引使安全关闭失效。未知 metadata 写结果保留 writer deadline，必须在 drain window 后恢复，防止迟到写入重新开放权限。
- Hasura CE v2.48 的 insert check 与 select row filter 独立：只请求 `affected_rows` 的写入不会被授权投影阻断；`returning`/`insert_one` 会回显由写权限接受的新行，然后该行仍可能无法通过普通 query 读取。应用不得把 mutation 回显当作投影读取授权证明；需要不可读候选写入的客户端必须固定只请求 `affected_rows`。
- migration `027_data_access_authorization_projections` 首次 stable release 的 API/Admin 与 manifest migration ceiling 曾同步为 `27`；当前完整 release 的 API floor/ceiling 和 manifest ceiling 均为 `28`。rollback gate 从独立 updater `0.2.0` 起提供，能力版本由镜像内编译常量确定且不能由环境变量覆盖；首次 migration `027` release 前必须先完成 updater-only bootstrap，且在推送镜像前通过 PostgreSQL 17 rollback-gate 集成门禁。bootstrap 占用独立稳定版本，只替换 updater；其 API/Admin/Worker digest 和 Compose 必须来自指定旧稳定 GitHub Release 的不可变双 Registry manifest/资产，并与 Registry tag 当前指向复核，不得从 tag 重新推导。bootstrap 不成为 GitHub latest，客户端仅在确认产品版本与数据库 migration 精确匹配后临时使用显式版本 manifest URL，且不执行 migration；后续更高版本完整 stable manifest 才恢复 latest。完整 manifest 生成器必须严格解析 `minUpdaterVersion` 并拒绝低于 `0.2.0` 的版本，不能只依赖 workflow 环境变量约定。updater 文件回滚先停止 API/Admin/Worker，migration `027+` 先持久启用 `file_rollback` gate，再在 gate 激活期间清理旧 holder，以全局 advisory lock 排空 mutation 并检查 v2 状态。具名 PostgreSQL holder 在有 gate 表时保持锁直至 gate 关闭，在 pre-027 无 gate 表时由 updater 显式终止；ready 探针无条件校验实际锁，覆盖旧文件恢复、旧 API 启动和健康验证。teardown 必须在数据库内复验具名 holder 后关闭 gate，并确认锁释放；任何预检/holder/teardown/健康验证失败都停止旧服务并保留原始备份 operation ID，阻断后续更新直到恢复。updater 重启只对文件可能已切换的 applying/verifying/rolling_back 状态要求回滚，备份未完成或文件未切换状态可重试常规操作，手工回滚恢复不能误用新操作 ID；恢复前校验备份文件齐全。migration down 在取得表独占锁前必须对 holder 和 gate 失败关闭，并设置有界锁等待。该查询兼容 pre-027 列结构且不依赖 update state；存在 v2 状态时拒绝启动旧发布文件并要求数据库备份恢复。生产激活前必须完成目标应用合同和真实 Project Session 联合验收；本地实现完成不等于已发布或已执行 OTA。

- Data Access v1 读取响应不得输出 `selectConstraint: null`，以便严格拒绝该字段的 v1 PUT 能直接回写；v2 读取和写入均要求非空投影约束。updater 回滚停止服务和数据库 gate/lock 探针必须使用 release Compose 固定的容器名直接执行 Docker 命令，避免切换后的 Compose 文件损坏时无法停止或冻结服务；恢复旧文件后才启动旧 Compose。
- Project Runtime Context 使用 migration `028_project_runtime_contexts` 的独立、审计化配置表，不能复用项目 schema environment 或可合并 JSON settings。migration `029_project_runtime_context_fences` 专门修复已应用早期 `028`、但因历史 migration 文件变更而缺少 fence 表的数据库，并从既有配置回填首次启用围栏；新 API 必须要求 `029`，不允许仅凭 `028` 启动。合法值固定为 `local / sandbox / testflight / production`；无记录表示兼容模式，损坏/读取失败必须拒绝而不是回退默认值。仅固定项目管理 capability `runtime_context:manage` 可变更配置。服务端在 RPC transaction 以内先以空值屏蔽连接或角色级 `druvia.service_environment`，再为已启用项目设置受管值；GraphQL、Functions internal GraphQL、Realtime JWT 和 Data Access HTTP/Realtime verifier 只从同一配置派生 `x-hasura-druvia-service-environment`。公开 Nginx 的 GraphQL/WS 直通路径删除客户端同名 Header，API proxy 不接受 caller Header。配置切换不追溯修改已签发 Realtime JWT，刷新 token 后才生效；如果将来需要即时撤销，必须另行设计连接回收。migration down 在存在任一运行环境记录或 fence 时拒绝；完整 release migration ceiling 为 `29`，但 updater-only bootstrap 仍可使用 migration `027`，因为它仅建立 migration `027+` 的回滚执行能力。
- 未知 Hasura 写结果回到 source metadata 仅在 resource version 与 preview 一致时才可按未应用处理；版本已变化必须 fail-closed。回滚 gate 关闭后若 holder 释放确认失败，应尝试重新启用持久 gate 并停止旧服务，保留恢复状态。
- 投影 target recovery 必须在标记 completed 之前逐表比对当前 Hasura scoped permission 与最新 baseline 列 grants 的物化结果；历史 target metadata 摘要相同也不能覆盖后续 reconcile 收缩的授权。回滚重试必须先保持或启用 gate，在 gate 激活期间清理旧 holder 并重建锁；预检拒绝时保留 gate、停止旧服务并等待人工恢复，不能短暂撤销既有围栏。
