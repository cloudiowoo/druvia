...
- 项目应用只通过三条固定 `SECURITY DEFINER` Hook 暴露 register/list/ack。Druvia 校验精确签名、项目 db_user owner、固定 search path、完整 ACL，并拒绝 owner 继承其他角色、任何非 superuser 角色直接或间接继承 owner、`REPLICATION`、其他 schema 的非 extension relation/column/sequence 权限或 CREATE，以及可直接调用的非 extension 外部 `SECURITY DEFINER` 函数；数据库管理员安装的 extension 自有对象不按业务跨 schema 权限处理。`RETURNS trigger/event_trigger` 不能由普通 SQL 直接调用，不计入 definer execute，但外部 relation `TRIGGER` 权限仍失败关闭。contract hash 与 binding 快照绑定，摘要复验和调用在同一 SQL statement 中完成。首版只支持 `db_user` 隔离到单一项目 schema；共享该用户的多环境部署不得启用，未来专用 NOLOGIN Hook owner 需另行设计角色生命周期。
